...
public class BitmapToVideoEncoder {
    ...
    private static float BIT_RATE = 0.0F;
    ...
    public BitmapToVideoEncoder(
            IBitmapToVideoEncoderCallback callback,
            int frameRate
    ) {
        mCallback = callback;
    ...
    public void startEncoding(int width, int height, File outputFile) {
        ...
        mediaFormat.setFloat(MediaFormat.KEY_BIT_RATE, BIT_RATE);
    ...
    private void encode() {
        ...
        while (!mNoMoreFrames || (mEncodeQueue.size() != 0)) {
